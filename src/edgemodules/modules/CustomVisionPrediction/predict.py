...
def extract_bilinear_pixel(img, x, y, ratio, xOrigin, yOrigin):
    ...
    b = xDelta * br + (1.0 - xDelta) * bl
    t = xDelta * tr + (1.0 - xDelta) * tl
    pixel = yDelta * t + (1.0 - yDelta) * b
    return pixel
...
def extract_and_resize(img, targetSize):
    ...
    resize_image = np.empty(
        (targetSize[0], targetSize[1], img.shape[2]), dtype=np.float32
    )
...
def preprocess_image_opencv(image_pil):
    ...
    log_msg(
        f"crop: {w}x{h}  to {crop_size}x{crop_size}, origin at {startx}, {starty}, target = {network_input_size}"
    )
    return cv2.resize(
        image[starty : starty + crop_size, startx : startx + crop_size],
        new_size,
